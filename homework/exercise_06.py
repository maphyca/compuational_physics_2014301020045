...
class fish(object):
    def __init__(self,jam=0.1,step=100):
        self.jam=jam
        self.n=0
        self.v=0
        self.a=0
...
for s in range(m):
    for i in range(n):
        ...
        Group[i].gfood=food(distance(Group[i].gv/n,Group[j].ga/n))
        ...
        Group[i].ini()
    plt.sca(ax1)
    plt.scatter(Groupv,Groupa,s=20,c=Groupfood,marker=(5,3),alpha=0.8,lw=2,facecolors="none")
    if s==(m/4-1):
        plt.sca(ax2)
        plt.scatter(Groupv,Groupa,s=20,c=Groupfood,marker=(5,3),alpha=0.8,lw=2,facecolors="none")
    if s==(m/2-1):
